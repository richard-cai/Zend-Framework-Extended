	   ...
	   $.fn.validate = function(options){
		 ...
		 this.each(function(){
			 ...
			 if(msg.length == 0){
				...
				$this.focus(function(){
					if($(this).next('span.'+o.msg_class).css('display') == 'none'){
						$(this).validate();
					}
				}).change(function(){
					if($(this).val().length == 0 || $(this).validate().data('jquery_validate_rs') == true){
						$(this).next('span.'+o.msg_class).css('display','none');
					}
				}).blur(function(){
			 ...
			 if(!first_time){
				 ...
				 if(o.required && jQuery.trim($this.val()).length == 0){
					 ...
					 msg.text(o.required_msg)
					 	...
					 	.animate({'opacity':1},o.ef.duration,function(){
					 		setTimeout(function(){
					 			if(msg.css('display') != 'none'){
					 				msg.fadeOut(o.ef.duration,function(){
