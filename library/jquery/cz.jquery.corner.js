...
$.fn.corner = function(options) {
	...
	if(settings.topOnly == undefined && settings.bottomOnly == undefined){
		settings.All = true;
	}
	...
	function applyCorners(box,settings)
	{
		...
		this.x_bgc 				= format_colour($$.css("backgroundColor"));		
		// Background Colour
		...
		*/
		for(var i in corners)
		{
			if(i > -1 < 4)
			{
				...
				var newCorner = document.createElement("div"),tp = (settings[cc].radius != undefined)?settings[cc].radius:0;
				
				$(newCorner).css({
					"position":"absolute",
					"font-size":"1px", 
					"overflow":"hidden"
				}).height(tp+ "px").width(tp + "px");
              ...
              */
              ...
              switch(cc)
              {
                  ...
                  case "tl":
                    ...
                    if(this.topContainer) var temp= this.topContainer.appendChild(newCorner);
					$(temp).attr("id","cctl");
                  ...
                  case "bl":
                    ...
                    if(newCorner.style.position == "absolute") newCorner.style.left = "0px";
					if(this.bottomContainer) var temp= this.bottomContainer.appendChild(newCorner);
					$(temp).attr("id","ccbl");
...
$(temp).attr("id","ccbottommiddlefiller");
    ...
    //  contentContainer.style.position = "absolute";
      ...
      if(settings.topOnly || settings.All){
    	  $(contentContainer).css({"borderTopWidth":"0"});   	
      }
      if(settings.bottomOnly || settings.All){
    	  $(contentContainer).css({"borderBottomWidth":"0"}); 
      }
